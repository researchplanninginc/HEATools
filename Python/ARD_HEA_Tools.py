...
def get_process_history (directory, inputLayer):
    ...
    xslTrans = scriptPath + "\\copy_geoprocessing_history.xslt"
    arcpy.XSLTransform_conversion(inputLayer, xslTrans, xmlDoc)
    # f = open(xmlDoc, "w")
...
def set_process_history (directory, inputLayer, process_history_text):
    ...
    xmlDoc = directory + "\\temp.xml"
    newxmlDoc = directory + "\\newtemp.xml"
    xslTrans = scriptPath + "\\exact_copy_of.xslt"
    arcpy.XSLTransform_conversion(inputLayer, xslTrans, xmlDoc)
    # f = open(xmlDoc, "w")
    # f.write("<metadata xml:lang=\"en\"></metadata>")
    # f.close()
    # arcpy.MetadataImporter_conversion(inputLayer, xmlDoc)
    # arcpy.ImportMetadata_conversion(inputLayer, "FROM_FGDC", xmlDoc)
    f = open(xmlDoc, "r")
    mdText = f.read()
    f.close()
    start = mdText.find("<Process")
    ...
    # arcpy.ImportMetadata_conversion(newxmlDoc, "FROM_FGDC", inputLayer)
    arcpy.Delete_management(xmlDoc)
